...
def get_course_analytics_data(course):
    ...
    if not data:
        ...
        # Discussion metrics
        discussion_threads = DiscussionThread.objects.filter(material__course=course).count()
        discussion_replies = DiscussionReply.objects.filter(thread__material__course=course).count()
